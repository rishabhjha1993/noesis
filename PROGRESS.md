...
Experiment A is **FAILED / CLOSED**. C1 Luna candidate research is **PASS**: Chuquicamata was strong end to end, and a controlled Noordoostpolder replay changed only Terra → Luna candidate research while freezing Stage 1, questions, verified identity context, prompt construction, web search, validation, and medium reasoning. Luna answered 4/5 versus Terra's 3/5, correctly answered the IJsseloog candidate, and cost `$0.08614012` versus approximately `$0.2605092` for Terra candidate research. The current highest-leverage bottleneck is Stage-1 identity-hypothesis formulation / SEE → IDENTITY translation. The next milestone is a controlled identity-hypothesis-formulation experiment; C2 identity-model comparison is deferred until upstream identity input is stabilized.
...
- No implementation work is currently in progress. The next experiment is controlled SEE → IDENTITY hypothesis formulation. C2, fallback/escalation, web-search optimization, Stage-3/Stage-1 substitutions, and representation compression are not in progress.
...
Design—but do not yet implement in this checkpoint—a controlled SEE → IDENTITY hypothesis-formulation experiment. Freeze already-seen regions/candidates and vary only how discriminative visible evidence is converted into a specific, falsifiable identity hypothesis. Do not inject known answers, weaken verification, start C2, add fallback/escalation, optimize web search, substitute Stage 3/Stage 1 models, or begin representation compression.
