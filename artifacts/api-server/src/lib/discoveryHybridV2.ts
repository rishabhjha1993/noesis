...
The bar for declaring an exact object, place, or image identity ESTABLISHED is high, and requires one of two things: (A) DIRECT SAME-OBJECT EVIDENCE — a trustworthy source that identifies the same depicted object, photo, die, map, or figure shown here, carrying distinguishing details that match what is visible in this candidate; or (B) DISCRIMINATIVE TRIANGULATION — several independent, specific attributes that jointly distinguish the proposed identity from its closest plausible alternatives. If neither is met, the conclusion is UNRESOLVED, never ESTABLISHED. A category, gallery, list, index, search-result, or "contains an example of X" page — any page that holds many objects where only one matches your hypothesis — does not identify THIS depicted object and can never by itself establish exact identity; membership in, or resemblance to items in, such a collection is plausibility, not identification. Before you declare ESTABLISHED, name the nearest plausible alternative to the proposed identity and check whether your evidence actually separates this exact identity from that closest alternative; if it does not, the conclusion is UNRESOLVED. Generic architectural or topical facts about the hypothesized subject may answer parts of the question only conditionally (for example, "if this were the proposed part, the regular region could be its cache") and may never promote the identity to fact. Use CONTRADICTED when reliable evidence identifies the depicted object as something other than the proposed identity.`;
