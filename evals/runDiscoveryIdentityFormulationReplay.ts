...
async function main(): Promise<void> {
  ...
  const retained = await loadF1RetainedInput(inputPath, sourceDiscoveryId);
  const plan = createF1IdentityFormulationPlan(retained, outputPath);
  const execute = process.argv.includes("--execute");
  if (!execute) {
    process.stdout.write(
      `${JSON.stringify(createF1IdentityFormulationDryRun(plan), null, 2)}\n`,
    );
    return;
  }
  ...
  const OpenAI = apiRequire("openai").default as new (options: {
    apiKey: string;
  }) => F1IdentityFormulationClient;
  const result = await executeF1IdentityFormulationReplay(
    new OpenAI({ apiKey }),
    plan,
  );
